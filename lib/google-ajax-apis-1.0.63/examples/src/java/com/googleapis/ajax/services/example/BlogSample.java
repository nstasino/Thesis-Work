...
public class BlogSample {
    ...
    private static void processCommandLine(CommandLine line, Options options) {
        if(line.hasOption(HELP_OPTION)) {
    		...
    		PagedList<BlogResult> response = query.withQuery(line.getOptionValue(QUERY_OPTION)).list();
	 ...
	 */
	private static void printResponse(PagedList<BlogResult> response) {
		...
		for (BlogResult result : response) {
			System.out.println(result.getTitle());			
			System.out.println(result.getContent());			
			System.out.println(result.getBlogUrl());			
			System.out.println("=======================================");			
		}
	}
    ...
    private static void printHelp(Options options) {
        int width = 80;
        String syntax = BlogSample.class.getName() + " <options>";
